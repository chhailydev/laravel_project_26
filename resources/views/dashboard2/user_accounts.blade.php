...
@section('content')
<div class="mt-[50px] relative overflow-x-auto shadow-md sm:rounded-lg">
    <div class="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 bg-white dark:bg-gray-800">
        <div class="relative">
            ...
            <form action="/dashboard/account" method="GET" class="inline">
                <input type="text" id="table-search-users" value="{{ old('query', $query ?? '') }}" name="query" class="block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for users">
            </form>
        </div>
            ...
            @if($accounts->count())
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                        @if($user->profile)
                        <img class="w-10 h-10 rounded-full" src="{{ route('profile_image', ['id' => $user->id]) }}" alt="Jese image">
                        @else
                        <img src="{{ asset('assets/noImage.jpg')}}" class="w-10 h-10 rounded-full" alt="">
                        @endif
                        <div class="ps-3">
                            <div class="text-base font-semibold">{{ $user->useranme }}</div>
                    ...
                    <td class="relative flex items-center justify-center">
                        <a href="/dashboard/edit/account/{{ $user->id }}">
                        ...
                        <form action="/dashboard/delete/account/{{ $user->id }}" method="POST" class="inline" onsubmit="return confirmDelete()">
                            @csrf
                            @method('DELETE')
...
@else
 <div class="flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
  ...
  <div>
    No users not found matching your seach query.
  </div>
</div>
@endif
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5">
                <form action="{{ route('registerUser')}}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-5">
                    @csrf
                    @method('POST')
                    <div class="file-upload" id="file-upload" style="width:90px; height:90px; border-radius: 50%;">
                        ...
                        <img src="#" id="file-preview" class="w-[90px] h-[90px] rounded-full" alt="Image Preview">
                    </div>
                    ...
                    <input type="email" name="email" id="email" placeholder="youremail@gmail.com" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required />
